...
export class AnalyticsController {
  ...
  trackVisitor = asyncHandler(async (req: Request, res: Response) => {
    const { source, referrer, page, path, sessionId: bodySessionId } = req.body;
    ...
    res.status(200).json({
      success: true,
      data: { visitorId: Number(visitor.id) }
    });
  });
  ...
  getAnalytics = asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    // Daily visitors - PostgreSQL compatible
    ...
    const formattedDailyVisitors = (dailyVisitors as any[]).map((item: any) => ({
      date: item.date,
    ...
    res.status(200).json({
      ...
      data: {
        summary: {
          ...
          bounceRate: totalVisitors > 0 ? Math.round((Number(totalVisitors) - Number(uniqueVisitors)) / Number(totalVisitors) * 100) : 0
        },
        sources: sources.map(s => ({
          source: s.source || 'direct',
          count: Number(s._count.source)
        })),
        pages: pages.map(p => ({
          page: p.page,
          views: Number(p._count.page)
        })),
        devices: devices.map(d => ({
          device: d.device || 'unknown',
          count: Number(d._count.device)
        })),
        dailyVisitors: formattedDailyVisitors,
  ...
  getSources = asyncHandler(async (_req: AuthRequest, res: Response) => {
    ...
    const formattedSources = sources.map(s => ({
      source: s.source || 'direct',
      count: Number(s._count.source),
      totalVisits: Number(s._sum.visitCount || 0)
    }));
